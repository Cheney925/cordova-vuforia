...
class RefFreeFrameGL {
    ...
    public RefFreeFrameGL(Activity activity) {
    ...
    public boolean init(int screenWidth, int screenHeight) {
        ...
        if ((shaderProgramID = SampleUtils.createProgramFromShaderSrc(FRAME_VERTEX_SHADER, FRAME_FRAGMENT_SHADER)) == 0)
            return false;
        
        if ((vertexHandle = GLES20.glGetAttribLocation(shaderProgramID, "vertexPosition")) == -1)
            return false;

        if ((textureCoordHandle = GLES20.glGetAttribLocation(shaderProgramID, "vertexTexCoord")) == -1)
            return false;

        if ((mvpMatrixHandle = GLES20.glGetUniformLocation(shaderProgramID, "modelViewProjectionMatrix")) == -1)
            return false;
    ...
    public void getTextures() {
        ...
        for (int i = 0; i < TEXTURE_NAME.TEXTURE_COUNT; i++)
            textures[i] = Texture.loadTextureFromApk(textureNames[i], mActivity.getAssets());
    ...
    void renderViewfinder() {
        ...
        GLES20.glDrawElements(GLES20.GL_TRIANGLE_STRIP, NUM_FRAME_INDEX, GLES20.GL_UNSIGNED_SHORT, indicesBuffer);
